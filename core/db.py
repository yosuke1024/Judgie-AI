...
def init_db():
    ...
    try:
        with engine.begin() as conn:
            conn.execute(
                text("ALTER TABLE hackathons ADD COLUMN re_evaluation_context_mode TEXT DEFAULT 'cumulative';")
            )
    except Exception:
        pass
    try:
...
def get_consultation_count(hackathon_id: int, team_id: str) -> int:
    with db_session() as db:
        count = (
            db.query(Evaluation).filter(Evaluation.hackathon_id == hackathon_id, Evaluation.team_id == team_id).count()
        )
        return count
...
def update_team_passcode(hackathon_id: int, team_id: str, new_passcode: str) -> bool:
    with db_session() as db:
        team_user = (
            db.query(User)
            .filter(User.hackathon_id == hackathon_id, User.team_id == team_id, User.role.in_(["team", "observer"]))
            .first()
        )
        if team_user:
...
def change_my_passcode(
    hackathon_id: int = None, team_id: str = None, current_passcode: str = None, new_passcode: str = None
) -> bool:
    ...
    with db_session() as db:
        ...
        if session_record:
            return {
                ...
                "hackathon_id": session_record.hackathon_id,
            }
        return None
...
def save_admin_chat(
    evaluation_id: int, question_en: str, question_ja: str, answer_en: str, answer_ja: str, qa_json: dict = None
) -> AdminChat:
    with db_session() as db:
        chat = AdminChat(
...
def delete_team(hackathon_id: int, team_id: str):
    with db_session() as db:
        ...
        # 2. Evaluation
        db.query(Evaluation).filter(Evaluation.hackathon_id == hackathon_id, Evaluation.team_id == team_id).delete(
            synchronize_session=False
        )

        # 3. Submission
        db.query(Submission).filter(Submission.hackathon_id == hackathon_id, Submission.team_id == team_id).delete(
            synchronize_session=False
        )

        # 4. Session
        db.query(Session).filter(Session.hackathon_id == hackathon_id, Session.team_id == team_id).delete(
            synchronize_session=False
        )

        # 5. User (Only delete teams/observers, do NOT delete admins/superadmins)
        db.query(User).filter(
            User.hackathon_id == hackathon_id, User.team_id == team_id, User.role.in_(["team", "observer"])
...
def delete_evaluation(hackathon_id: int, evaluation_id: int):
    with db_session() as db:
        ...
        eval_record = (
            db.query(Evaluation).filter(Evaluation.id == evaluation_id, Evaluation.hackathon_id == hackathon_id).first()
        )
        if eval_record:
...
def seed_demo_data():
    ...
    with db_session() as db:
        ...
        # Create Hackathon
        demo_h = Hackathon(id=9999, name="Judgie Demo Hackathon", max_consultations=3)
        db.add(demo_h)
        db.flush()
        ...
        # Create Admin
        demo_admin = User(hackathon_id=9999, team_id="demo_admin", passcode=hash_passcode("demo123"), role="admin")
        db.add(demo_admin)
        ...
        c1_scores = {
            ...
            "Problem Solving & Impact": 3.0,
            "Product & UX": 3.0,
            "Working Prototype": 3.0,
            "Presentation": 3.5,
        }
        c1_strengths_risks = {
            ...
            "judges_feedback": [
                {
                    "judge_name": "Alex",
                    "judge_role": "Serial Entrepreneur",
                ...
                {
                    "judge_name": "David",
                    "judge_role": "Principal Software Engineer",
                ...
                {
                    "judge_name": "Lisa",
                    "judge_role": "Head of Product Design",
                ...
                {
                    "judge_name": "Sarah",
                    "judge_role": "Senior Product Manager",
        ...
        eval_c1 = Evaluation(
            hackathon_id=9999,
            team_id="demo_team",
            scores_json=json.dumps(c1_scores),
            impact_score=3.0,
            strengths_risks_json=json.dumps(c1_strengths_risks),
            is_final=False,
            source_text="Demo Team Consultation 1 Source Code",
        )
        db.add(eval_c1)
        ...
        eval_c2 = Evaluation(
            hackathon_id=9999,
            team_id="demo_team",
            scores_json=json.dumps(c2_scores),
            impact_score=3.5,
            strengths_risks_json=json.dumps(c2_strengths_risks),
            qa_json=json.dumps(c2_qa),
            is_final=False,
            source_text="Demo Team Consultation 2 Source Code",
        )
        db.add(eval_c2)
        ...
        c3_scores = {
            ...
            "Technical Implementation": 4.0,
            "Problem Solving & Impact": 4.0,
            "Product & UX": 4.0,
            "Working Prototype": 4.0,
            "Presentation": 4.5,
        }
        ...
        eval_c3 = Evaluation(
            hackathon_id=9999,
            team_id="demo_team",
            scores_json=json.dumps(c3_scores),
            impact_score=4.0,
            strengths_risks_json=json.dumps(c3_strengths_risks),
            is_final=False,
            source_text="Demo Team Consultation 3 Source Code",
        )
        db.add(eval_c3)
        ...
        eval_final = Evaluation(
            ...
            is_final=True,
            source_text="Demo Team Final Submission Source Code",
        )
        db.add(eval_final)
        db.flush()
        ...
        eval_t2_c1 = Evaluation(
            hackathon_id=9999,
            team_id="demo_team2",
            scores_json=json.dumps(t2_c1_scores),
            impact_score=2.8,
            strengths_risks_json=json.dumps(t2_c1_strengths_risks),
            is_final=False,
            source_text="Demo Team 2 Consultation 1",
        )
        db.add(eval_t2_c1)
        ...
        eval_t2_c2 = Evaluation(
            hackathon_id=9999,
            team_id="demo_team2",
            scores_json=json.dumps(t2_c2_scores),
            impact_score=3.5,
            strengths_risks_json=json.dumps(t2_c2_strengths_risks),
            is_final=False,
            source_text="Demo Team 2 Consultation 2",
        )
        db.add(eval_t2_c2)
        ...
        eval_t3_final = Evaluation(
            hackathon_id=9999,
            team_id="demo_team3",
            scores_json=json.dumps(t3_final_scores),
            impact_score=4.2,
            strengths_risks_json=json.dumps(t3_final_strengths_risks),
            is_final=True,
            source_text="Demo Team 3 Final Submission",
        )
