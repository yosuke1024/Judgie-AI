...
with tab6:
    ...
    st.warning(t(
        ...
        "⚠️ **警告 / 注意**: 複数の言語・口調を設定すると、AIがそれぞれの表現でフィードバックやアクションアイテムを作成するため、レスポンスの生成時間（遅延）が長くなります。最適なパフォーマンスのために、必要な言語のみ（通常は1〜2つ）を設定することをお勧めします。"
    ))

    current_langs = get_ai_response_languages(current_h_id)

    with st.form("ai_languages_form"):
        st.write(t("Configure AI Response Languages", "AIレスポンス言語設定"))
        langs_input = st.text_input(
            t("Languages (comma-separated, max 5)", "言語・口調設定（カンマ区切り、最大5つ）"),
            value=", ".join(current_langs),
            help=t("e.g., Japanese, English, Kansai-ben, Gal-go", "例: 日本語, 英語, 関西弁, ギャル語")
        )

        submitted_langs = st.form_submit_button(t("Save Language Settings", "言語設定を保存"), type="primary")
        if submitted_langs:
            parsed_langs = [l.strip() for l in langs_input.split(",") if l.strip()]
            if not parsed_langs:
                st.error(t("Please enter at least one language.", "少なくとも1つの言語・口調を入力してください。"))
            elif len(parsed_langs) > 5:
                st.error(t("You can specify a maximum of 5 languages.", "最大で5つの言語・口調まで設定可能です。"))
            else:
                set_ai_response_languages(current_h_id, parsed_langs)
