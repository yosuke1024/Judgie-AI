...
st.markdown(
    t(
        "Overview of the project heat and team rankings.",
        "プロジェクト全体の熱量と、全チームの暫定・最終スコアの俯瞰ビューです。",
    )
)
...
try:
    if current_h_id:
        users = (
            db.query(User).filter(User.role == "team", User.hackathon_id == current_h_id).order_by(User.team_id).all()
        )
    else:
...
if data:
    df = pd.DataFrame(data).sort_values(
        by=[t("Total Score", "総合スコア"), t("Product / Team", "プロダクト / チーム")], ascending=[False, True]
    )
        ...
        for i, c in enumerate(criteria):
            with tabs[i]:
                ...
                st.dataframe(
                    ...
                    hide_index=True,
                    column_config={
                        "Score": st.column_config.ProgressColumn("Score", format="%.1f", min_value=0, max_value=5.0)
                    },
                )
...
st.markdown(
    t(
        "This expert panel will evaluate your submissions.",
        "本プロジェクトでは、以下の5名のAI審査員が容赦のない評価を行います。",
    )
)
    ...
    for i, p in enumerate(active_personas):
        with cols[i]:
            with st.container(border=True):
                ...
                st.markdown(
                    f'<div style="display: flex; align-items: center; margin-bottom: 10px;">{avatar_html}<h3 style="margin: 0;">{p["name"]}</h3></div>',
