...
tab1, tab2 = st.tabs([t("🤖 Gemini Configuration", "🤖 Gemini設定"), t("🔒 Change Password", "🔒 パスワード変更")])
...
with tab1:
    ...
    api_key_input = st.text_input(
        t("Tenant Gemini API Key", "テナント用 Gemini APIキー"),
        type="password",
    ...
    if st.button(
        t("Save & Validate API Key", "APIキーを検証して保存"), type="primary", key="save_api", disabled=is_demo
    ):
        if api_key_input:
            with st.spinner(
                t(
                    "Validating key and fetching available models...",
                    "キーの有効性を検証し、利用可能なモデルを取得中...",
                )
            ):
                try:
                    import json
                    ...
                    if models:
                        ...
                        st.error(
                            t(
                                "No supported models returned. Please check the key.",
                                "対応するモデルが見つかりませんでした。キーを確認してください。",
                            )
                        )
                    ...
                    st.error(
                        t(
                            ...
                            f"APIキーの検証に失敗しました。入力が正しいか確認してください: {str(e)}",
                        )
                    )
        else:
    ...
    if current_key:
        ...
        if st.button(
            t("Save Model Settings", "モデル設定を保存"), type="primary", key="save_model_settings", disabled=is_demo
        ):
...
with tab2:
    ...
    if is_demo:
        st.caption(
            t("💡 Password change is disabled in Demo Mode.", "💡 デモモードではパスワード変更は無効化されています。")
        )
    with st.form("change_admin_pass_form"):
        ...
        confirm_pass = st.text_input(
            t("Confirm New Password", "新しいパスワード（確認）"), type="password", disabled=is_demo
        )
