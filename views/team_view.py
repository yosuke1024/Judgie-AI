    ...
    with db_session() as db:
        if current_h_id:
            users = (
                ...
                .filter(User.role == "team", User.hackathon_id == current_h_id)
...
with col1:
    ...
    with db_session() as db:
        result = (
            db.query(Hackathon.name)
            .join(User, User.hackathon_id == Hackathon.id)
            .filter(User.team_id == view_team_id)
            .first()
        )
            ...
            with p_col1:
                with st.popover(t("⚙️ Edit Profile", "⚙️ プロフィールの編集")):
                    with st.form("profile_form"):
                        ...
                        o_liner = st.text_input(
                            t("One-liner (Catchphrase)", "一言アピール（キャッチコピー）"),
                            value=profile.get("one_liner", ""),
                        )
                ...
                with st.popover(t("🔐 Change Password", "🔐 パスワード変更")):
                    with st.form("change_team_pass_form"):
                        ...
                        confirm_pass = st.text_input(
                            t("Confirm New Password", "新しいパスワード（確認）"), type="password"
                        )
    ...
    if role in ("admin", "observer"):
        ...
        with sub_c2:
            ...
            st.button(
                t("Submit Final Pitch", "最終成果物として提出"),
                type="primary",
                disabled=True,
                key="demo_final_btn",
                use_container_width=True,
        ...
        with st.expander(
            t("💡 Tips for ZIP creation (to avoid size limits)", "💡 ZIPファイル作成時のヒント (容量超過を防ぐために)"),
            expanded=False,
        ):
            st.markdown(
                t(
        ...
        if video_enabled:
            ...
            uploader_label = t(
                "Artifacts (ZIP, PDF) (Video uploads are disabled)",
                "成果物ファイル (ZIP, PDF) (動画のアップロードは無効化されています)",
            )
        ...
        cons_left_str = (
            t("Unlimited", "無制限") if max_consultations == -1 else f"{consultations_left} / {max_consultations}"
        )
        ...
        with sub_c1:
            ...
            if st.button(
                t("Get AI Coaching", "AIコーチングを受ける"),
                type="secondary",
                disabled=not can_consult or not uploaded_files,
            ):
        ...
        if submit_type:
            ...
            with st.status(
                t("🤖 AI Expert Panel is reviewing...", "🤖 AI専門家パネルが審査中..."), expanded=True
            ) as status:
                try:
                    # Execute evaluation flow via service layer
                    st.write(
                        t("📦 Processing files and running AI panel...", "📦 ファイルを処理し、AIパネルを実行中...")
                    )
    ...
    if not eval_rows:
        ...
        for i, crit in enumerate(criteria):
            ...
            with col:
                ...
                st.caption(
                    t(
                        f"Contribution: {contribution:.1f} / {max_contrib:.1f} pts",
                        f"貢献スコア: {contribution:.1f} / {max_contrib:.1f}点",
                    )
                )

            chart_data.append({"Criteria": crit["name"], "Score": s})
        ...
        for idx, lang_name in enumerate(languages):
            ...
            with lang_tabs[idx]:
                ...
                if action_items:
                    ...
                    st.write(
                        t("No specific action items provided.", "具体的なアクションアイテムは提供されていません。")
                    )
                        ...
                        for idx, lang_name in enumerate(languages):
                            ...
                            with qa_tabs[idx]:
                                ...
                                for j in msg_data.get("judges_responses", []):
                                    ...
                                    st.markdown(
                                        f'<div style="display: flex; align-items: center; margin-bottom: 10px;">{get_avatar_html(j_name, j_icon, size=24)}<strong style="margin-left: 8px;">{j_name}</strong></div>',
                                        unsafe_allow_html=True,
                                    )
            ...
            if has_reached_limit:
                ...
                if role == "team":
                    if is_demo:
                        ...
                        turns_left_str = (
                            f"{max_qa - turns_used} / {max_qa}" if max_qa != -1 else t("Unlimited", "無制限")
                        )
                        st.markdown(
                            t(
                        ...
                        with st.form("objection_form"):
                            obj_text = st.text_area(
                                t("Your message to the judges:", "審査員へのメッセージ:"), height=150
                            )
                            submit_obj = st.form_submit_button(
                                t("Send Message ✊", "メッセージを送信 ✊"), type="primary"
                            )
                            ...
                            if submit_obj:
                                if not obj_text.strip():
                                    ...
                                    with st.status(
                                        t("⚖️ Judges are discussing your point...", "⚖️ 審査員があなたの意見を議論中..."),
                                        expanded=True,
                                    ) as status:
                                        try:
                                            ...
                                            status.update(
                                                label=t(
                                                    "✅ Judges have responded!", "✅ 審査員からの回答が届きました！"
                                                ),
                                                state="complete",
                                                expanded=False,
                                            )
