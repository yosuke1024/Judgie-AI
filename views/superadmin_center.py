...
with col2:
    st.subheader(t("🏢 Existing Projects", "🏢 既存のプロジェクト一覧"))

    db = SessionLocal()
    try:
        results = (
            db.query(Hackathon.id, Hackathon.name, Hackathon.created_at, User.team_id.label("admin_id"))
    ...
    if not rows:
        st.info(t("No projects exist yet.", "まだプロジェクトは作成されていません。"))
    else:
        data = []
        for r in rows:
            data.append(
